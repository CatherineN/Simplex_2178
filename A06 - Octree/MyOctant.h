...
namespace Simplex
{
	...
	class MyOctant
	{
		...
		static uint m_uMaxLevel;//stores the max level an octant can go to
		static uint m_uIdealEntityCount; //how many Entities this octant will ideally contain

		uint m_uID = 0; //the current ID for this octant
		uint m_uLevel = 0; //current level of the octant
		uint m_uChildren = 0;// # of children on the octant (either 0 or 8)
		...
		vector3 m_v3Center = vector3(0.0f, 0.0f, 0.0f); //center point of the octant
		vector3 m_v3Min = vector3(0.0f, 0.0f, 0.0f); //min vector of the octant
		vector3 m_v3Max = vector3(0.0f, 0.0f, 0.0f); //max vector of the octant
		...
		std::vector<MyOctant*> m_lChild; //list of nodes that contain objects (will be applied to the root only)
